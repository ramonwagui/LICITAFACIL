...
export async function GET(request, { params }) {
  try {
    ...
    const result = await sql`SELECT * FROM contratos WHERE id = ${params.id}`;
...
export async function PUT(request, { params }) {
  try {
    ...
    const result = await sql`UPDATE contratos SET ${setClause} WHERE id=${params.id} RETURNING *`;
    return Response.json(result[0]);
  } catch (err) {
...
export async function DELETE(request, { params }) {
  try {
    autenticarToken(request);
    await sql`DELETE FROM contratos WHERE id = ${params.id}`;
